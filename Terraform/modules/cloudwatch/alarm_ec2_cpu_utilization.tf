...
resource "aws_cloudwatch_metric_alarm" "ec2_cpu_utilization_debian12" {
    alarm_name = "${lower(var.tags.name)}-ec2-cpu-utilization-debian12"
    alarm_description = "CPU utilization of EC2 instance ${var.ec2_instances.debian12.id}"
